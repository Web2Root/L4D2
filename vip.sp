...
public Plugin myinfo = {
  ...
  author = "Aleexxx",
  description = "VIP-система для ASC, оружия, ближнего боя и т. д. с использованием SQLite",
  version = "1.0.6",
  url = "http://draen.org"
}
...
public Action Command_drop(int client, int args) {
  ...
  if (IsValidPlayer(client)) {
    ...
    if (counters[USE_VIP][client] >= 1 && counters[USE_DROP][client] == 1) { 	
      ...
      CPrintToChat(client, "\x05[\x04VIP\x05]\x03 Если вы снова сбросите оружие из VIP, вы будете оштрафованы на 0 квоты.");
    ...
    // Verificando si el jugador ya ha utilizado el vip mas de dos veces y ha dropeado el arma mas de dos veces
      ...
      // Imprimiendo mensaje al cliente
      CPrintToChat(client, "\x05[\x04VIP\x05]\x03 Вы выпустили более двух орудий. Вы будете оштрафованы на 0 квот.");
      // Quitando puntos al jugador
